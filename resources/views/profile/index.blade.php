                            ...
                            @else
                                ...
                                @elseif($user->hasPendingFriendRequestFor(auth()->user()))
                                    <div class="space-x-2">
                                        ...
                                        <form action="{{ route('friends.patch', ['user_id' => $user]) }} " method="post" class="inline">
                                            @csrf
                                            @method('PATCH')
                                            <button class="text-indigo-600 inline">Accept</button>
                                        </form>

                                        <form action="{{ route('friends.destroy', ['user_id' => $user]) }} " method="post" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button class="text-indigo-600 inline">Reject</button>
                                        </form>
